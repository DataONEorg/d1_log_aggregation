...
/**
 * Quartz Job that starts off the recovery of log entries from a
 * from a CN
 *
 * Job may not be executed concurrently for a coordinating node
 *
 * An executable task that will query a CN that is fully recovered and pull
 * all the dates of items needed back to the localhost
 * The process will then call LogEntryIndexTask for the batches of
 * LogEntrySolrItems that are returned
 *
 * The main reason to keep this a separate class, instead of integrating
 * the code directly into LogAggregationRecoverJob, is for testing.
 * Another, but less important reason, is that it conforms to the pattern
 * of Quartz jobs spawning off a separate task that performs the work
 * The main difference is that this task is not distributed.
 *
 * If the task fails for any reason in the middle of recovery
 * then the records on CN may become out of sync with those reported by the MN
 *
 * @author waltz
 */
